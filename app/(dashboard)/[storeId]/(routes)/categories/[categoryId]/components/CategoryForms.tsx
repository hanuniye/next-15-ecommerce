...
type CategoryFormVales = z.infer<typeof formSchema>;
...
const CategoryForms = ({ initialData, billboards }: CategoryFormsProps) => {
  ...
  const form = useForm<CategoryFormVales>({
  ...
  const onSubmit = async (value: CategoryFormVales) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(
